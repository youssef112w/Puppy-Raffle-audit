...
Prepared by: [Ateed](https://github.com/youssef112w/youssef112w)
Lead Auditors: 
- Ateed
...
# Executive Summary
The PuppyRaffle audit focused on the core lottery logic within `PuppyRaffle.sol`. The protocol was found to have a critical vulnerability that could lead to significant loss of funds for the owner due to integer truncation errors. While the core lottery mechanism functions as intended, the fee collection logic is insecure, leading to data loss when the number of participants exceeds a specific threshold. 

We also identified a need for better event emission practices to improve the transparency and observability of state changes for off-chain integrations.
## Issues found
...
## Medium
### [M-1]  Looping through players arrays to check for duplicates in `PuppyRaffle::enterRaffle` is a potential denail of service (dos).incrementing of gas cost for  future enterance. 
...
Place this  following into `puppyRaffleTest.t.sol`
```js
   function testDosForEnterRaffle() public {
        vm.txGasPrice(1);
        ...
        uint256 gasUsedFirst = (gasStart - gasCost) * tx.gasprice;
        ...
        uint256 gasCostTwo = gasleft();
        uint256 gasUsedSecond = (gasStartTwo - gasCostTwo) * tx.gasprice;
...
We may be tempted to suggest something like "Don't check for duplicates.", but it's important to preserve the original functionality as much as possible. If we do suggest a change in functionality, we must be clear in explaining why.
...
### [I-1] Titile:Unspecific Solidity Pragma
